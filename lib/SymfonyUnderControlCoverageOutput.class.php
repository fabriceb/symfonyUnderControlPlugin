...
<metrics files="' . count($this->tests) . '" functions="0" cls="' . count($this->tests) . '" clsa="0" clsc="0" roots="0" leafs="0" interfs="0" maxdit="0">';
    foreach ( $this->tests['Unit'] as $test )
    {
      $xml .= '<file name="' . $test->getFilename() . '" classes="1" functions="0" loc="0" cloc="0" ncloc="0" locExecutable="0" locExecuted="0" coverage="' . $test->getCoverage() . '">
